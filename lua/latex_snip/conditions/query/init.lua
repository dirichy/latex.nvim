...
--- when n=nil, it will return the number k such that the node is in k-th arg if the cursor is in any arg of cmd.
--- --TODO:when n=0, it will return the number k such that the node is in k-th arg if the cursor is in optional arg of cmd.
--- check_ancestor means whether to check ancestor. For example, \cmd_name{\othercmd{aa|a}}
--- @param cmd_name string|string[]|table<string,boolean> - when cmd_name is a table, will check all cmd_name in the table. return the number k such that the node is in k-th arg if at least one of them is found
